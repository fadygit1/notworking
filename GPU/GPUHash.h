...
__device__ __noinline__ void _GetHash160(uint64_t *x, uint64_t *y, uint8_t *hash) {
  ...
  // Create a buffer for the public key (65 bytes: 0x04 + 32 bytes X + 32 bytes Y)
  uint8_t pubkey[65];
  pubkey[0] = 0x04; // Uncompressed public key prefix
  
  // Copy X coordinate (big-endian)
  for (int i = 0; i < 4; i++) {
    uint64_t val = x[i];
    pubkey[1 + i*8] = (val >> 56) & 0xFF;
    pubkey[2 + i*8] = (val >> 48) & 0xFF;
    pubkey[3 + i*8] = (val >> 40) & 0xFF;
    pubkey[4 + i*8] = (val >> 32) & 0xFF;
    pubkey[5 + i*8] = (val >> 24) & 0xFF;
    pubkey[6 + i*8] = (val >> 16) & 0xFF;
    pubkey[7 + i*8] = (val >> 8) & 0xFF;
    pubkey[8 + i*8] = val & 0xFF;
  }
  
  // Copy Y coordinate (big-endian)
  for (int i = 0; i < 4; i++) {
    uint64_t val = y[i];
    pubkey[33 + i*8] = (val >> 56) & 0xFF;
    pubkey[34 + i*8] = (val >> 48) & 0xFF;
    pubkey[35 + i*8] = (val >> 40) & 0xFF;
    pubkey[36 + i*8] = (val >> 32) & 0xFF;
    pubkey[37 + i*8] = (val >> 24) & 0xFF;
    pubkey[38 + i*8] = (val >> 16) & 0xFF;
    pubkey[39 + i*8] = (val >> 8) & 0xFF;
    pubkey[40 + i*8] = val & 0xFF;
  }
  
  // First SHA-256 (on the public key)
  SHA256Initialize(s);
  
  // Process first 64 bytes (0x04 + 60 bytes of X)
  for (int i = 0; i < 16; i++) {
    w[i] = (pubkey[i*4] << 24) | (pubkey[i*4+1] << 16) | (pubkey[i*4+2] << 8) | pubkey[i*4+3];
  }
  SHA256Transform(s, w);
  
  // Process remaining 1 byte of X + 32 bytes of Y + padding
  w[0] = (pubkey[64] << 24) | 0x800000; // Last byte of Y + padding bit
  ...
  // Length in bits (65 bytes * 8 = 520 bits)
  w[15] = 520;
  
  // Second SHA-256 transform
  SHA256Transform(s, w);
  
  // Convert SHA-256 hash to big-endian for RIPEMD-160
  for (int i = 0; i < 8; i++) {
    s[i] = bswap32(s[i]);
  }
  
  // Prepare for RIPEMD-160
  *(uint64_t *)(s + 8) = 0x80ULL; // Padding
  *(uint64_t *)(s + 10) = 0ULL;   // More padding
  *(uint64_t *)(s + 12) = 0ULL;   // More padding
  *(uint64_t *)(s + 14) = ripemd160_sizedesc_32; // Length in bits (256)
