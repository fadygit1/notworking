...
// forward declaration
static void threadFunctionStartingKeys(Secp256K1* secp, int threadId, int groupSize, int numThreadsGPUperCPU,
    Int tRangeDiffPerGPUThread, Int* tRangeStartInThread, Int* privateKeys, Point* publicKeys);
...
#if 0 // duplicate definition disabled
    ...
    Int tRangeDiffPerGPUThread, Int* tRangeStartInThread, Int* privateKeys, Point* publicKeys)
{
    ...
    for (int i = 0; i < numThreadsGPUperCPU; i++) {
        // compute end key in range (not currently used but kept for clarity)
        tRangeEndInThread.Set(&tRangeStartInThread[threadId]);
        tRangeEndInThread.Add(&tRangeDiffPerGPUThread);

        // starting private key for this GPU lane
        Int& pk = privateKeys[threadId * numThreadsGPUperCPU + i];
        pk.Set(&tRangeStartInThread[threadId]);

        // enforce repetition limits
        g_patternSkipper.skipInvalid(pk);

        // compute middle point public key
        Int middleKey(&pk);
        middleKey.Add((uint64_t)(groupSize / 2));
        publicKeys[threadId * numThreadsGPUperCPU + i] = secp->ComputePublicKey(&middleKey);

        // move to next sub-range
