...
void printUsage() {
	...
	fprintf(stdout, "VanitySearch [-check] [-u] [-b] [-stop] [-stopAll] [-i inputfile]\n");
	...
	fprintf(stdout, " -stopAll: Stop when all addresses are found and exit\n");
...
int main(int argc, char* argv[]) {
	...
	int repeatLimits[6] = {0,0,0,0,0,0};
	...
	// --- GPU availability detection (CPU fallback) ---
	int cudaDevices=0;
	bool gpuAvailable=true;
	if(cudaGetDeviceCount(&cudaDevices)!=cudaSuccess || cudaDevices==0){
		gpuAvailable=false;
		fprintf(stderr,"[WARN] No CUDA device detected, switching to CPU-only mode.\n");
	}
	...
	while (a < argc) {
		...
		if (strcmp(argv[a], "-gpu") == 0) {
			a++;
			if(!gpuAvailable){ fprintf(stderr,"[WARN] -gpu requested but no CUDA device present, option ignored.\n"); }
		}
		...
		else if (strcmp(argv[a], "-stopAll") == 0) {
			stopAllMode = true;
			a++;
		}
			...
			a++; 
			repeatLimits[1] = getInt("r1", argv[a]); 
			a++; 
		}
		else if (strcmp(argv[a], "-r2") == 0) { 
			a++; 
			repeatLimits[2] = getInt("r2", argv[a]); 
			a++; 
		}
		else if (strcmp(argv[a], "-r3") == 0) { 
			a++; 
			repeatLimits[3] = getInt("r3", argv[a]); 
			a++; 
		}
		else if (strcmp(argv[a], "-r4") == 0) { 
			a++; 
			repeatLimits[4] = getInt("r4", argv[a]); 
			a++; 
		}
		else if (strcmp(argv[a], "-r5") == 0) { 
			a++; 
			repeatLimits[5] = getInt("r5", argv[a]); 
			a++; 
		}
	...
	fprintf(stdout, "GPU available: %s\n", gpuAvailable?"yes":"no");
	fflush(stdout);

	VanitySearch* v = new VanitySearch(secp, address, searchMode, (stop||stopAllMode), outputFile, maxFound, bc);
	v->Search(gpuId, gridSize);
	std::array<int,6> lim; 
	for(int i=0;i<6;i++) 
		lim[i]=repeatLimits[i]; 
	g_patternSkipper.setLimits(lim);
